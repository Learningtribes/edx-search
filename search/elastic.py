...
def _translate_hits(es_response, total_keys=None):
    ...
    if "aggregations" in es_response and "total_records" in es_response["aggregations"]:
        # Total number without Filters ( also without `Course.org` / `Program.orgs` )
        ...
        if total_keys is not None:
            ...
            # For Developer/Super Admin, we may apply the total number filtered by `Course.org` / `Program.orgs`
